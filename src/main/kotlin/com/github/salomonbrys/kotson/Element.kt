...
inline fun <reified T : Any> JsonElement.castTo(): T = Gson().fromJson(this, typeToken<T>())

inline fun <reified T : Any> JsonElement.getObject(key: String): T = Gson().fromJson(get(key), typeToken<T>())
inline fun <reified T : Any> JsonElement.getObject(index: Int): T = Gson().fromJson(get(index), typeToken<T>())
...
fun JsonObject.getAsJsonArrayOrNull(key: String) = get(key) as? JsonArray

operator fun JsonArray.contains(value: Any): Boolean = contains(value.toJsonElement())

fun JsonArray.getAsJsonObject(index: Int) = get(index) as JsonObject
